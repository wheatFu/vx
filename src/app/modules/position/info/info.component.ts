...
export class InfoComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  constructor(
    ...
    private pmService: PositionService,
  ...
  getTreeDat() {
    this.pmService.getOrganizationTree().subscribe(res => {
      if (+res.result.code === 0) {
        ...
        this.nodeOpts = data
        this.setMaxLevel(this.getTreeMaxHeight(data))
      }
    })
  }
  getTreeMaxHeight(data: TreeSourceOpts): number {
    /********方式一start***/
    // const queue = []
    // const node_list = []
    ...
    //     queue.push.apply(queue,children.map(item => ({ ...item, level:node.level + 1 })))
    //   }
    //   level = node.level
    // }
    // return level
    /********方式一end***/
    // 方式二
    if (data == null) return 0 // 如果是个空树，则树的高度则为0
    const { subOrganizations: children } = data
    if (!children || !children.length) return 1
    const _tree_height = []
    for (const child of children) {
      _tree_height.push.call(_tree_height, this.getTreeMaxHeight(child))
    }
    return Math.max.apply(null, _tree_height) + 1 // +1 加的是根节点的高度
  }
